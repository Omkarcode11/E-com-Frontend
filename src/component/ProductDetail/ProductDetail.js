...
function ProductDetail() {
  ...
  const {setCheckout,isAuthenticated} = useContext(AppContext)
  ...
  function cartHandler() {
    ...
    let prod = {
      ...
      cart = JSON.parse(localStorage.getItem("rajdhaniCart"));
      else cart = (localStorage.getItem('rajdhaniCart'))
    ...
    <div className="product-detail-layout">
      <div className="product-detail">
        <div className="product-detail-img">
          <img src={product.img} className="" alt="Card title" />
        </div>
        <div className="product-details-text">
          <div className="card-body">
            <h5 className="card-title h1 py-2">{product.name}</h5>
            <h6 className="card-title h4 pb-3">
              Price ₹{product.price} <del>{product.price * 1.5}</del>
              <sup> 25% off</sup>
            </h6>
            ...
            <div className="product-detail-button ">
              <Link
                to={isAuthenticated==='false'?'/auth':"/checkout"}
